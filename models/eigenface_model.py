...
class EnhancedEigenFaceRecognizer:
    """
    增强版EigenFace识别器
    """
    def __init__(self, n_components=None, variance_ratio=0.98, knn_neighbors=5):
        ...
        Parameters:
            n_components (int): PCA组件数，如果为None则根据方差比例自动确定
            variance_ratio (float): 保留的方差比例，用于自动确定组件数
            knn_neighbors (int): k-NN的邻居数
        """
    ...
    def train(self, train_images, train_labels):
        ...
        with tqdm(total=100, desc="PCA训练", unit="%") as pbar:
            ...
            # 实际PCA训练
            self.projected_faces = self.pca.fit_transform(centered_data)
        ...
        print(f"\n✅ 训练完成!")
        print(f"📈 PCA解释方差比例: {explained_variance:.4f} ({explained_variance*100:.2f}%)")
    ...
    def get_model_info(self):
        ...
        return {
            ...
            'explained_variance': sum(self.pca.explained_variance_ratio_) if self.pca else 0
        }